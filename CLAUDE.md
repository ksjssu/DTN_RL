...
- **Compile the project**: `compile.bat` (Windows) or `./compile.sh` (Linux/Unix)
  ...
  - If builds drift, delete `target/` and rerun compile script
...
### Testing
- **Run all tests**: `java -cp "target;lib/*;target/test" test.AllTests` (after compiling with `./compile.sh`)
- Test files are located in `src/test/` and mirror the structure of `src/`
- Tests use JUnit 3/4 and class names end with `*Test`
...
- `scenarios/dynamic/` - DRL-specific scenarios organized by experiment type:
  - `drl_train/` - Training scenarios for different buffer sizes (5M-50M)
  - `drl_eval/` - Evaluation scenarios for different buffer sizes
  - `prophet/` - Prophet router baseline scenarios
  - `heuristic_d010/`, `heuristic_d020/`, `heuristic_d030/` - Heuristic baseline comparisons
...
- **Line length**: Keep near 100 characters
- **Language**: Restrict to Java 8 features
- **Naming**:
  - Packages: lowercase
  - Classes: UpperCamelCase
  - Methods and fields: lowerCamelCase
  - Constants: UPPER_SNAKE_CASE
- **Documentation**: Provide Javadoc for public APIs; use inline comments only for subtle logic
- **Testing**: Write deterministic JUnit tests; seed randomness explicitly for reproducibility

### Project Organization
- Production code: `src/` (organized by package: `core/`, `routing/`, `movement/`, `report/`, etc.)
- Test code: `src/test/` (mirrors structure of `src/`)
- Dependencies: `lib/` (staged JARs - never commit new JARs without discussion)
- Build output: `target/` (never commit - generated by compile scripts)
- Data and scenarios: `data/`, `scenarios/`

### Commit Guidelines
- Use imperative, scoped commit messages (e.g., "routing: tighten relay policy")
- Never include `target/` outputs or introduce new JARs without approval
- Run `./compile.sh` and tests before submitting changes
...
### DRL Workflow
The DRL integration uses a client-server architecture where the Java simulator acts as the client and communicates with a Python-based PPO agent server:

1. **State Observation**: Java simulator ([RLBridgeReport.java](src/report/RLBridgeReport.java)) sends state observations via HTTP POST to Python server
   - 5-dimensional state: `[contacts_norm, pred, bufocc_mean, capacity_norm, self_buf_util]`
   - Previous transition data (state, action, reward) for training
2. **Action Selection**: Python PPO agent ([drl_server.py](toolkit/drl_server.py)) processes observations and returns actions
   - Actions are predictability delta values within `[-delta_limit, +delta_limit]`
   - Training mode: stochastic policy with exploration
   - Evaluation mode: deterministic policy (mean action)
3. **Action Application**: Java applies deltas to Prophet routing protocol predictability values
   - Modified predictability affects routing decisions in [ProphetRouter.java](src/routing/ProphetRouter.java)
4. **Reward Calculation**: Buffer-specific reward functions compute rewards based on:
   - Delivered messages (primary reward)
   - Relayed messages (with buffer-specific bonuses/penalties)
   - Buffer occupancy and network pressure (for small buffers)
5. **Learning**: PPO agent updates policy using collected transitions (state, action, reward, next_state)

### DRL Configuration
- **Environment Variables**: Control training behavior
  - `TRAIN_UNTIL_SECONDS` - Training duration limit
  - `SAVE_AT_SECONDS` - Checkpoint save interval
  - `EVAL_ONLY` - Run in evaluation-only mode (no training)
  - `DRL_HOST`, `DRL_PORT` - Server connection settings
  - `STEP_REWARD_TRACKING` - Enable detailed step-by-step reward logging
  - `PPO_DEBUG_ACTIONS` - Enable action debugging logs
