...
Request JSON:
{
  ...
  "time": 3600,
  "delta_limit": 0.05,
  "prev_transition": [ {"host":"p1","dest":"w82","relayed":3,"drops":1,"aborted":0,"delivered":1,"buffer_size":10485760}, ... ],
  "state_batch": [ {"host":"p1","dest":"w82","buffer_size_mb":10.0,
...
- Per-step updates combine buffer-dependent weights: r = w_del(B)*delivered + w_relay(B,pressure)*relayed + w_abort(B)*aborted. Rewards are assigned back to the messages that received actions in the previous step for that host.
...
if not TORCH_OK:
    raise SystemExit(
        "PyTorch is required for DRL mode but could not be imported "
        f"({TORCH_IMPORT_ERROR or 'unknown error'}). "
        "Install torch to run DRL simulations."
    )
...
def log_step_rewards(sim_time, rewards_per_host, buffer_size_mb=None, episode=0):
    ...
    try:
        ...
        with open(log_file, "a", encoding="utf-8") as f:
            ...
            f.write(f"{episode},{sim_time},{total_reward},{avg_reward},{num_hosts}\n")
    except Exception:
...
# Reward scaling (buffer-dependent, overridable via environment)
W_DELIVER_FALLBACK = float(os.environ.get("W_DELIVER", "1.0"))
W_RELAY_FALLBACK   = float(os.environ.get("W_RELAY", "0.0"))
W_ABORT_FALLBACK   = float(os.environ.get("W_ABORT", "0.0"))
W_DELAY            = 0.0  # penalty for high delay (kept for compatibility)
...
ABORT_GAIN        = float(os.environ.get("ABORT_GAIN", "0.8"))
ABORT_SLOPE       = float(os.environ.get("ABORT_SLOPE", "0.18"))
ABORT_MID         = float(os.environ.get("ABORT_MID", "25.0"))

DEFAULT_BUFFER_MB = float(os.environ.get("DEFAULT_BUFFER_MB", "30.0"))
...
def buffer_weight_profile(buffer_mb):
    """
    Returns (delivery_weight, relay_weight, abort_weight) for the given buffer size.
    When buffer size cannot be inferred, fall back to static weights.
    """
    if buffer_mb is None:
        return W_DELIVER_FALLBACK, W_RELAY_FALLBACK, -W_ABORT_FALLBACK

    sigmoid = 1.0 / (1.0 + math.exp(-RELAY_SLOPE * (buffer_mb - RELAY_MID)))
    relay_weight = RELAY_MIN + (RELAY_MAX - RELAY_MIN) * sigmoid
    abort_weight = -ABORT_GAIN / (1.0 + math.exp(ABORT_SLOPE * (buffer_mb - ABORT_MID)))
    return W_DELIVER_FALLBACK, relay_weight, abort_weight


def relay_pressure_coeff(buffer_mb, pressure_diff):
    """
    Scales pressure difference to [-1, 1] and blends it with a buffer-size gate.
    Small buffers dampen the coefficient, large buffers approach 1.
    """
    gate = 0.5 * (1.0 + math.tanh(PRESSURE_SLOPE * (buffer_mb - PRESSURE_MID)))
    if PRESSURE_CLIP > 0:
...
class PPOPolicy:
    def __init__(self, obs_dim=5, hidden1=128, hidden2=128):
        ...
        self.clip_eps = float(os.environ.get("PPO_CLIP", "0.25"))
        self.value_coef = float(os.environ.get("PPO_VALUE_COEF", "0.5"))
        self.entropy_coef = float(os.environ.get("PPO_ENTROPY", "0.01"))
        self.lr = float(os.environ.get("PPO_LR", "3e-4"))
        eff_batch = int(os.environ.get("PPO_BATCH_SIZE", "128"))
        if eff_batch < 32:
            eff_batch = 32
        self.batch_size = eff_batch
        default_minibatch = max(16, eff_batch // 2)
        self.minibatch = int(os.environ.get("PPO_MINIBATCH", str(default_minibatch)))
        self.epochs = int(os.environ.get("PPO_EPOCHS", "4"))

        self.device = torch.device("cpu")

        # Actor: outputs mean; log_std is a learnable parameter
        # Architecture: 5-128-128-1 (uses local observation only)
    ...
    def _construct_global_obs(self, local_obs, global_features):
        ...
        Args:
            ...
            global_features: Tensor of shape (batch_size, 10) - global network features

        Returns:
            Tensor of shape (batch_size, 15) - combined global observation
        """
        if global_features is None or global_features.size(1) == 0:
            # Fallback: if no global features provided, pad with zeros
            batch_size = local_obs.size(0)
            zeros = torch.zeros(batch_size, 10, device=self.device)
            return torch.cat([local_obs, zeros], dim=1)
    ...
    def act_batch(self, obs_batch, delta_limit, keys, global_features=None):
        """
        CTDE: Actor uses local obs (5D), Critic uses global obs (15D).

        Args:
            obs_batch: Local observations (batch_size, 5)
            delta_limit: Action scaling limit
            keys: List of (host, message) keys
            global_features: Optional global network features (batch_size, 10)
        """
        if len(obs_batch) == 0:
            return []

        # Local observation for Actor (5D)
        local_obs = self._to_tensor(obs_batch)

        # Actor uses LOCAL observation only
        mean = self.actor(local_obs).squeeze(-1)
        ...
        # CTDE: Critic uses GLOBAL observation (local + global features)
        if global_features is not None:
            global_feat_tensor = self._to_tensor(global_features)
        else:
            # Fallback: zero-padding for global features (Java not updated yet)
            batch_size = local_obs.size(0)
            global_feat_tensor = torch.zeros(batch_size, 10, device=self.device)

        global_obs = self._construct_global_obs(local_obs, global_feat_tensor)
        val = self.critic(global_obs).squeeze(-1).detach()

        # store last step actions per key (store both local and global obs)
        act_list = action_scaled.cpu().tolist()
        for i, k in enumerate(keys):
            self.last_actions[k] = (local_obs[i].detach(), global_obs[i].detach(),
                                    u[i].detach(), logp[i].detach(), val[i].detach())
        return act_list

    def _add_to_buffer(self, local_obs, global_obs, u_pre, logp, val, rew):
        """CTDE: Store both local obs (for Actor) and global obs (for Critic)."""
    ...
    def update(self, rewards_per_host, rewards_per_key, mapping_host_to_keys, delta_limit, epochs=None):
        ...
        for host, acted_keys in acted_by_host.items():
            ...
            for k in acted_keys:
                # CTDE: unpack 5 values (local_obs, global_obs, u_pre, logp, val)
        ...
        # CTDE: Prepare tensors - separate local and global observations
        local_obs = torch.stack(self.buf_local_obs)  # Actor uses this (5D)
        global_obs = torch.stack(self.buf_global_obs)  # Critic uses this (15D)
        ...
        for _ in range(self.epochs):
            for start in range(0, n, mb):
                ...
                b = idx[start:end]
                # CTDE: Actor uses local obs, Critic uses global obs
                ...
                # Actor loss: uses local observation (5D)
                ...
                # Critic loss: uses global observation (15D)
        ...
        # Clear buffer after update (CTDE: clear both local and global obs buffers)
        self.buf_local_obs.clear()
        self.buf_global_obs.clear()
        self.buf_act_pre.clear()
        self.buf_logp.clear()
        self.buf_val.clear()
        self.buf_rew.clear()
...
class Handler(BaseHTTPRequestHandler):
    ...
    EP_COUNT = int(os.environ.get("EPISODE_OFFSET", "0"))  # increments when simulator notifies episode end
    ...
    def do_POST(self):
        ...
        if self.path == "/episode_end":
            ...
            ppo_reward = Handler.EP_REWARD_ACC
            Handler.EP_COUNT += 1
            try:
                ...
                with open(os.path.join(report_dir, "drl_episode_rewards.txt"), "a", encoding="utf-8") as f:
                    f.write(
                        f"episode {Handler.EP_COUNT} reward {reward} ppo_reward {ppo_reward:.6f} "
                        f"avg_delivery_rate {avg:.6f} delivered {delivered} created {created} sim_id {sim_id}\n"
                    )
        ...
        # Extract default buffer size from sim_id (fallback)
        default_buffer_size = extract_buffer_size_mb(sim_id)
        if default_buffer_size is None:
            default_buffer_size = DEFAULT_BUFFER_MB
        ...
        for item in prev_tr:
            try:
                ...
                # Get host-specific buffer size from prev_transition or use cached value
                buffer_size_bytes = item.get("buffer_size", None)
                if buffer_size_bytes is not None:
                    buffer_size = float(buffer_size_bytes) / (1024.0 * 1024.0)
                else:
                    # Try to get from cached features
                    feature = prev_features.get(key) if key else None
                    buffer_size = feature.get("buffer_size_mb") if (feature and "buffer_size_mb" in feature) else default_buffer_size

                # Calculate buffer-specific weights
                w_del, w_relay_weight, w_abort_weight = buffer_weight_profile(buffer_size)

                key_reward = 0.0

                if delivered > 0:
                    key_reward += w_del * delivered
                    Handler.EP_DELIVERED_SUM += delivered

                if aborted > 0:
                    abort_penalty = w_abort_weight * aborted
                    key_reward += abort_penalty
                    Handler.EP_OVERHEAD_PENALTY += abs(abort_penalty)
                ...
                if relayed > 0:
                    ...
                    if feature:
                        ...
                        pressure_diff = self_util_prev - buf_mean_prev
                    pressure_coeff = relay_pressure_coeff(buffer_size, pressure_diff)
                    relay_bonus = (w_relay_weight * relayed) + (CONDITIONAL_RELAY_GAIN * pressure_coeff * relayed)
                    key_reward += relay_bonus
                    Handler.EP_PRESSURE_SUM += abs(relay_bonus)

                if key_reward != 0.0:
                    rewards_per_host[host] = rewards_per_host.get(host, 0.0) + key_reward
                    if key:
                        rewards_per_key[key] = rewards_per_key.get(key, 0.0) + key_reward
            except Exception:
                continue

        # 2) Prepare obs batch for current state and mapping to keys
        obs_batch = []
        global_features_batch = []  # CTDE: Global features for each observation
        keys = []
        ...
        for item in state_batch:
            try:
                ...
                # Extract buffer size (MB) for this host
                buffer_size_mb = _to_float(item.get("buffer_size_mb"), default_buffer_size)
                ...
                # CTDE: Extract global features (10D)
                global_feat = [
                    _to_float(item.get("global_total_nodes", 0.0), 0.0),
                    ...
                    _to_float(item.get("global_std_buf", 0.0), 0.0),
                    _to_float(item.get("global_delivery_rate", 0.0), 0.0),
                    _to_float(item.get("global_sys_pressure", 0.0), 0.0),
                    _to_float(item.get("global_avg_contacts", 0.0), 0.0),
                    _to_float(item.get("global_relay_ratio", 0.0), 0.0),
                    _to_float(item.get("global_sim_progress", 0.0), 0.0),
                ]

                # Local observation (5D): [contacts_norm, pred, bufocc_mean, capacity_norm, self_buf_util]
                obs_batch.append([c, p, buf_mean_cur, cap_norm_cur, self_util_cur])
                global_features_batch.append(global_feat)
                k = f"{host}#{dest}"
                keys.append(k)
                ...
                next_features[k] = {
                    ...
                    "freebuf_norm": f,
                    "buffer_size_mb": buffer_size_mb,  # Cache buffer size for next step
                }
            except Exception:
                continue
        ...
        if STEP_REWARD_TRACKING and rewards_per_host:
            buffer_size = extract_buffer_size_mb(sim_id)
            # episode index: completed episodes + current ep offset (ep_idx starts at 0)
            ep_num = Handler.EP_COUNT + ep_idx + 1
            log_step_rewards(sim_time, rewards_per_host, buffer_size, episode=ep_num)
        ...
        # 4) Infer actions for current batch (CTDE: pass global features)
        if not TORCH_OK:
            actions_raw = Handler.AGENT.act_batch(obs_batch, delta_limit)
        else:
            # CTDE: Pass global_features_batch to act_batch
            actions_raw = Handler.AGENT.act_batch(obs_batch, delta_limit, keys, global_features=global_features_batch)
