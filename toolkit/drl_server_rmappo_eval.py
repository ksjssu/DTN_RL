...
def _candidate_report_dirs(buffer_size_mb=None):
    dirs = []
    env_dirs = os.environ.get("REPORT_DIRS", "")
    if env_dirs:
        for d in env_dirs.split(os.pathsep):
            ...
            if d:
                dirs.append(d)
    if buffer_size_mb:
        buf = int(buffer_size_mb)
        dirs.extend([
            os.path.join("reports_drl_train", f"buf{buf}"),
            os.path.join("reports_drl_eval", f"buf{buf}"),
            os.path.join("reports", f"buf{buf}"),
        ])
    dirs.extend(["reports_drl_train", "reports_drl_eval", "reports"])
...
def _find_report_file(sim_id, report_class, buffer_size_mb=None):
    filename = f"{sim_id}_{report_class}.txt"
    for d in _candidate_report_dirs(buffer_size_mb):
...
def read_final_report_metrics(sim_id, buffer_size_mb=None):
    """Parse final avg latency and overhead ratio from report files."""
    avg_delay = None
    overhead = None

    latency_path = _find_report_file(sim_id, "LatencyPerIntervalReport", buffer_size_mb)
    overhead_path = _find_report_file(sim_id, "OverheadPerIntervalReport", buffer_size_mb)

    if latency_path:
        line = _read_last_data_row(latency_path)
        if line:
            parts = line.split()
            if len(parts) >= 4:
                try:
                    avg_delay = float(parts[3])
                except Exception:
                    avg_delay = None

    if overhead_path:
        line = _read_last_data_row(overhead_path)
        if line:
            parts = line.split()
            if len(parts) >= 5:
                try:
...
def log_episode_reward(episode_num, sim_id, total_reward, avg_delivery_rate=None,
    ...
    try:
        ...
        if avg_overhead is None or avg_delay is None:
            parsed_delay, parsed_overhead = read_final_report_metrics(sim_id, buffer_size_mb)
            if avg_overhead is None:
                avg_overhead = parsed_overhead
            if avg_delay is None:
                avg_delay = parsed_delay
