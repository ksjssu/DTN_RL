#!/usr/bin/env python3
"""Plot hourly average network buffer utilization for buf20 rng1 runs 1-21."""
...
def write_svg_plot(
    ...
    x_min = min(all_hours)
    x_max = max(all_hours)
    y_min = min(all_values + [y_low, y_high])
    y_max = max(all_values + [y_low, y_high])
    ...
    y_low_svg = y_to_svg(y_low)
    y_high_svg = y_to_svg(y_high)
    lines.append(
        f'<line x1="{margin_left}" y1="{y_high_svg:.2f}" x2="{margin_left + plot_w}" y2="{y_high_svg:.2f}" stroke="#333" stroke-dasharray="6,4" />'
    )
    lines.append(
        f'<text x="{margin_left + plot_w - 6}" y="{y_high_svg - 6:.2f}" font-size="12" text-anchor="end" fill="#222">upper 0.7</text>'
    )
    lines.append(
        f'<line x1="{margin_left}" y1="{y_low_svg:.2f}" x2="{margin_left + plot_w}" y2="{y_low_svg:.2f}" stroke="#333" stroke-dasharray="6,4" />'
    )
    lines.append(
        f'<text x="{margin_left + plot_w - 6}" y="{y_low_svg - 6:.2f}" font-size="12" text-anchor="end" fill="#222">lower 0.4</text>'
    )
...
def main() -> None:
    ...
    if len(series) < len(runs):
        raise SystemExit("Missing required RLStateReport data for runs 1-14.")

    colors = rainbow_red_to_purple(len(series))
    out_path = out_dir / "buf20_rng1_runs1_4_hourly_bufutil.svg"
    write_svg_plot(series, colors, out_path, y_low=0.4, y_high=0.7)
